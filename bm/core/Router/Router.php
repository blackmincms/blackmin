...
class Router {
    ...
    public function createInstanceFrom(array $payload): self {
        ...
        if ($action && $url) {
            return $this->routerFactory($payload["action"], $payload["url"], $payload["parm"] ?? '');
        }
    ...
    public function delegate() {
        ...
        if ($this->action === "load") {
            if (is_string($this->parm) && $this->parm !== '') {
                $path = $this->pathDirectory . "/" . $url . "/" . $this->parm;
                $ext = '.php';
                if ($this->parm === "filter") {
                    $ext = '.phtml';
                }
        ...
        $class = $this->delegatedClassFactory($url, [$this->database, $this->action, $this->parm]);
        $result = $class->parse();
    ...
    public function autoDelegate() {
        ...
        $class = 'Controller\\' . $url; // TODO tutaj sprawdzić czy dobry namespace
        $object = new $class($this->database, $this->action, $this->parm);

        // rex commponents
        if ($url !== '') {
            $component = $url;
            $obj = new $component($this->database, $this->action, $this->parm); // TODO tutaj pewnie ma się coś dalej wydarzyć albo do skasowania
        }
