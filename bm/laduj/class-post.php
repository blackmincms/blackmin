...
/*
	...
	ten plik służy do ładowanie posta/postów
	...
	class post_bm {
		...
		public function load_post_bm() {	
			...
			if($bm_url["checked_url"] == "root"){
				$wynik = $db_bm->query("SELECT * FROM `_prefix_bm_data_posty` WHERE (`status`LIKE 'public' OR `status`LIKE 'protect_password' OR `status`LIKE 'private') ORDER BY `id` DESC LIMIT ". $this->load_post, false);
			...
			}elseif($bm_url["checked_url"] == "search_page"){
			...
			}elseif($bm_url["checked_url"] == "post_page"){
				// sprawdzanie czy dev szuka innego posta/postów
				if($this->search_post === null){
					$s = $bm_url["path"];
				}else{
